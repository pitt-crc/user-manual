...
connection to the [**GlobalProtect VPN**](https://services.pitt.edu/TDClient/33/Portal/KB/ArticleDet?ID=3426)
(see [Step 1](../getting-started/getting-started-step1-account.md)). With this
connection established, you can proceed with the steps below.

Clients running MacOS can use the built-in Terminal app (in Applications/Utilities) or [**iTerm2**](https://iterm2.com/). Clients running Windows can use [**MobaXterm**](https://mobaxterm.mobatek.net/) or [**PuTTY**](https://www.chiark.greenend.org.uk/~sgtatham/putty/latest.html) to
...
The syntax to connect to the CRCD login node from your terminal command line is
...
Use `htc.crc.pitt.edu` instead to reach the HTC cluster. The `-X` option enables X forwarding for applications that
generate a GUI such as `xclock`.
...
If this is the first time that your client machine is logging into `h2p.crc.pitt.edu`, you will see a warning that "The authenticity of the host ... can't be established" and if you wish to continue connecting. Answer: yes.
...
!!! example "Successfully logged in to login node"
    === "MacOS"
        ![iTerm2 terminal session after logging in to a CRCD login node](../_assets/img/web-portals/iTerm2.png)

    === "Windows"
        ![MobaXterm terminal session after logging in to a CRCD login node](../_assets/img/web-portals/MobaXterm.png)
...
- **Linux shell** -- a text-based user-interface that interprets user commands and scripts. CRCD supports `bash` and `csh`, with `bash` being the default
