...
!!! note "Jobs use a minimum of two nodes"
    ...
    single-node workloads, submit to the [SMP](smp.md) cluster instead.
    Running a single-node job here still reserves the unused
    node(s), and those resources are counted against your Resource Allocation usage.
